...
for(i in 1:8){
  mydata <- read.csv(paste("C:\\Users\\shrut\\Desktop\\美的\\数据\\气温数据\\SURF_CLI_CHN_MUL_DAY-TEM-12001-",as.character(fillname[i]),"07.TXT",sep=""),header=FALSE,sep="")
  b <- aggregate(mydata$V8,list(mydata$V1),FUN=mean)#每一年的1月平均气温
  c <- rbind(a,b)#10年拼接
}
names(c) <- c("sitesnum","meanJulytem")
decadetem <- aggregate(meanJulytem~sitesnum,c,FUN = mean)##计算10年的7月平均气温
